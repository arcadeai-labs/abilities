...
async function call(method: string, path: string, body?: unknown) {
  ...
  const parsed = response.headers.get("content-type")?.includes("json")
    ? JSON.parse(text)
    : text
  // `parsed` is whatever the endpoint returned; each block below declares the
...
line("GET /api/types?toolkit=Math   — how you learn what `math` is")
{
  ...
  console.log(
    `  ${status}  snapshot=${headers.get("x-catalog-snapshot")}  ${source.split("\n").length} lines`
  )
  console.log(
    `  ${source
      .split("\n")
      .find((l) => l.includes("add(input:"))
      ?.trim()}`
  )
  console.log(
    `  imports in the declarations: ${/^\s*import /m.test(source) ? "yes" : "none"}`
  )
}
...
line("GET /api/coverage")
{
  ...
  const pct = (t: { tools: number; typed: number }) =>
    `${((100 * t.typed) / t.tools).toFixed(1)}%`
  console.log(
    `  ${status}  ${r.totals.tools} tools / ${r.totals.toolkits} toolkits`
  )
  console.log(
    `  curated ${pct(r.curated)} typed · generated ${pct(r.generated)} typed`
  )
}
...
line("POST /api/validate   — a run that contradicts its own output schema")
{
  ...
  for (const d of r.diagnostics) {
    console.log(
      `  ${d.code} at run line ${d.start.line}: ${d.message.slice(0, 110)}`
    )
  }
}
...
line(`PUT /api/scripts/${NAME}   — upsert, twice`)
{
  ...
  console.log(
    `  first ${first.status} (created)   second ${second.status} (replaced, v${s.version})`
  )
...
line(`GET /api/scripts/${NAME}   — every aspect, straight from the database`)
{
  ...
  const s: Record<string, unknown> = body
...
line("POST run   — input the contract rejects")
{
  ...
  const report: { outcome: unknown } = body
  console.log(`  ${status}  ${JSON.stringify(report.outcome)}`)
}
...
line("GET /api/openapi")
{
  ...
  const document: { paths: Record<string, unknown> } = body
