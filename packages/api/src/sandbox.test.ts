...
const raw = (body: string) => `var __config = { run: async function (input, ctx) {\n${body}\n} };`;
...
`),
  ...
  it("exposes only the granted tools, so an ungranted one is not even a property", async () => {
    const outcome = await run(
      raw(`  return { hasMath: typeof ctx.math, hasGithub: typeof ctx.github };`),
      { grant: ADD },
    );
...
`),
    ...
    expect(outcome.result).toMatchObject({ kind: "ok", output: { blocked: true } });
...
describe("the bridge", () => {
  it("carries a real call to Arcade and back", async () => {
    const outcome = await run(raw(`  return { sum: await ctx.math.add({ a: "2", b: "3" }) };`), {
      grant: ADD,
    });

    expect(outcome.result).toEqual({ kind: "ok", output: { sum: "5" } });
    expect(outcome.toolCalls).toMatchObject([{ path: "math.add", qualifiedName: "Math.Add", ok: true }]);
  });
  ...
  it("crosses nested arguments intact, as JSON rather than a live object", async () => {
    ...
    const outcome = await run(
      raw(`  return { total: await ctx.math.sumList({ numbers: ["1", "2", "3.5"] }) };`),
...
`),
    ...
    expect(outcome.result).toMatchObject({ kind: "ok", output: { threw: true } });
...
describe("limits", () => {
  it("stops an unbounded loop at the deadline", async () => {
    const outcome = await run(raw(`  while (true) {}`), { limits: { timeoutMs: 400 } });

    expect(outcome.result).toMatchObject({ kind: "limit_exceeded", limit: "cpu" });
  ...
  for (var i = 0; i < 1e9; i++) held.push(new Array(10000).fill("x"));`),
    ...
    expect(outcome.result).toMatchObject({ kind: "limit_exceeded", limit: "memory" });
  });
  ...
  return { done: true };`),
    ...
    expect(outcome.result).toMatchObject({ kind: "limit_exceeded", limit: "tool_calls" });
  ...
  it("bounds a real host call that outlives the deadline", async () => {
    ...
    const outcome = await run(raw(`  return await ctx.math.add({ a: "2", b: "3" });`), {
      grant: ADD,
      limits: { timeoutMs: 5 },
    });
  ...
  it("caps output size", async () => {
    ...
    expect(outcome.result).toMatchObject({ kind: "limit_exceeded", limit: "output_bytes" });
  });
});
...
describe("results", () => {
  ...
  it("surfaces a thrown guest error as a script error, not a crash", async () => {
    ...
    expect(outcome.result).toMatchObject({ kind: "script_error", name: "TypeError", message: "nope" });
  });
});
