...
function camel(name: string): string {
  const cleaned = name.replace(
    /[^A-Za-z0-9]+(.)?/g,
    (_, next: string | undefined) => (next ? next.toUpperCase() : "")
  )
...
export type NameMap = {
  ...
  namespaces: Map<
    string,
    { toolkitName: string; methods: Map<string, ToolBinding> }
  >
...
export function buildNameMap(tools: readonly NameInput[]): NameMap {
  ...
  const sorted = [...tools].sort(
    (a, b) =>
      a.toolkitName.localeCompare(b.toolkitName) || a.name.localeCompare(b.name)
  )
  ...
  for (const tool of sorted) {
    ...
    const taken = new Map<string, unknown>(namespaces)
    if (reserved) taken.set(preferred, null)
  ...
  for (const tool of sorted) {
    // Both were populated by the loop above, over the same `sorted` array.
    ...
    if (namespace === undefined || entry === undefined) {
      throw new Error(`no namespace was claimed for ${tool.toolkitName}`)
    }
    ...
    if (byQualifiedName.has(tool.qualifiedName)) {
      warnings.push(
        `duplicate qualified name \`${tool.qualifiedName}\`; kept the first`
      )
      continue
    }
