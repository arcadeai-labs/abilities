...
export type ScriptParams = {
  /**
   * JSON Schema for the run input. Arrives over HTTP as an opaque object;
   * {@link specFromJsonSchema} is what narrows and rejects it.
   */
  input: JsonSchema | Record<string, unknown>
  /** JSON Schema the return value must satisfy. */
  output: JsonSchema | Record<string, unknown>
  /**
...
export function assemble(params: ScriptParams): AssembleResult {
  ...
  if (typeof params.run !== "string" || params.run.trim() === "") {
    diagnostics.push(
      contractError(
        "`run` must be the method source, e.g. `async run(input, { log }) { … }`.",
        "contract/missing-run"
      )
    )
  }
  ...
  return {
    ok: true,
    assembled: { source, contract, runLineOffset: preamble.length },
  }
}
...
export function contractFrom(params: ScriptParams): {
  ...
  const read = (schema: unknown, label: string): Spec | null => {
    try {
      return specFromJsonSchema(schema, label)
    } catch (error) {
      if (!(error instanceof JsonSchemaError)) throw error
      diagnostics.push(
        contractError(`\`${label}\`: ${error.message} (at ${error.path})`)
      )
      return null
    }
  }
 ...
 */
export function toAuthorCoordinates(
  ...
  return {
    ...
    start: { ...diagnostic.start, line: diagnostic.start.line - runLineOffset },
      ...
      ...diagnostic.end,
      line: Math.max(1, diagnostic.end.line - runLineOffset),
    },
  }
}
