...
describe("accepts", () => {
  it("a script that conforms to its contract", async () => {
    const result = await validateScript({
      input: {
        type: "object",
        properties: { owner: { type: "string" }, repo: { type: "string" }, issue: { type: "integer" } },
...
describe("rejects", () => {
  ...
  it("aliasing a toolkit, which would hide the call from grant extraction", async () => {
    const result = await validateScript(script(`  const g = github;\n  await g.whoAmI({});`));
  ...
  it("computed tool access, for the same reason", async () => {
    const result = await validateScript(script(`  await github["who" + "AmI"]({});`));
  ...
  it("type-level computation, which would let a script slow the checker down", async () => {
    const result = await validateScript(
      script(`  type Explode<T> = T extends [infer H, ...infer R] ? [Explode<R>, Explode<R>] : T;`),
    );
  ...
  it("a contract that is not usable JSON Schema", async () => {
    const result = await validateScript(
      script(`  log("hi");`, { input: { type: "array" } as JsonSchema }),
    );
