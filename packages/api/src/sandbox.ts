  ...
  try {
    const log = context.newFunction("__log", (handle) => {
      ...
      if (
        ...
        logBytes + text.length > limits.maxLogBytes
      ) {
        exceeded ??= "logs"
        return
      }
    ...
    const callTool = context.newFunction(
      "__callTool",
      (pathHandle, argsHandle) => {
        ...
        if (toolCalls.length >= limits.maxToolCalls) {
          exceeded ??= "tool_calls"
          reject(
            `This script may make at most ${limits.maxToolCalls} tool calls.`
          )
          return deferred.handle
        }
        ...
        void Promise.race([
          ...
          new Promise<never>((_, rejectRace) => {
            timer = setTimeout(() => {
              exceeded ??= "cpu"
              rejectRace(
                new Error(`"${path}" did not finish before the run deadline.`)
              )
            }, remaining)
          }),
        ]).then(
    ...
    if (evaluated.error) {
      const dumped: { name?: string; message?: string } | undefined =
        context.dump(evaluated.error)
    ...
    if (exceeded) {
      return finish({
        kind: "limit_exceeded",
        limit: exceeded,
        message: `Exceeded the ${exceeded} limit.`,
      })
    }

    const output: unknown = JSON.parse(outputJson)
    return finish({ kind: "ok", output })
  } finally {
