...
function createHost(files: Map<string, string>): ts.CompilerHost {
  const isLib = (fileName: string) =>
    fileName.startsWith(LIB_DIR) && fileName.endsWith(".d.ts")
...
function closest(
  ...
  for (const candidate of candidates) {
    // Indices stay within `previous`/`row` by construction; the fallbacks are
    // for the type system, not for a case that can occur.
    ...
    for (let i = 1; i <= target.length; i++) {
      ...
      for (let j = 1; j <= candidate.length; j++) {
        row[j] = Math.min(
          ...
          (previous[j - 1] ?? Number.POSITIVE_INFINITY) +
            (target[i - 1] === candidate[j - 1] ? 0 : 1)
        )
      }
      previous = row
    }
    const distance = previous[candidate.length] ?? Number.POSITIVE_INFINITY
...
export async function validateScript(
  ...
  if (
    ...
    Buffer.byteLength(params.run, "utf8") > MAX_RUN_BYTES
  ) {
    return {
      ...empty,
      diagnostics: [
  ...
  const scoped = namespaces.flatMap(
    (namespace) => catalog.byNamespace.get(namespace) ?? []
  )
  ...
  if (!scriptFile) {
    throw new Error(
      `the program lost ${SCRIPT_PATH}, which it was created with`
    )
  }
  ...
  for (const diagnostic of [
    ...
    const to = scriptFile.getLineAndCharacterOfPosition(
  ...
  const diagnostics = raw.map((diagnostic) =>
    toAuthorCoordinates(diagnostic, runLineOffset)
  )
  ...
  const byQualifiedName = new Map(
    catalog.rows.map((row) => [row.qualifiedName, row] as const)
  )
  ...
  for (const namespace of namespaces) {
    if (Object.keys(grant).some((path) => path.startsWith(`${namespace}.`)))
      continue
    diagnostics.push({
      category: "contract",
